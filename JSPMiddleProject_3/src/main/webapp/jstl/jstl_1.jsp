...
<%--
		JSTL
			       ...
			       변수
			        ...
			        제어문
			        	...
			        	<c:forEach var="i" begin="1" end="10" step="1">
			        		반복 수행 문장    1부터 10까지 1씩 증가
			        	</c:forEach>
			        	단점은 감소식이 없다
			        	<for:frEach var="vo" items="${list}">
			        		가장 많이 사용되는 forEach문
			        	</c:forEach>
			        	
			        	<c:forTokens var="변수명" items="값" delims="구분자">
			        		StringTokenizer st= new StringTokenizer(값,구문자)
			        		=> 메뉴, 이미지 ...
			        	</c:forTokens>
			...
			format
					...
					<fmt:formatNumber> => DecimalFormat => 숫자 변환
						...
						<fmt:formatNumber value="${price}" pattern="###,###">
											30,000
											------ portone(포트온) => 실제 숫자가 있어야 한다
			function : String이 가지고 있는 메소드만 지원
			...
			<%@ taglib prefix="fmt" uri="jakarta.tags.format" %>
